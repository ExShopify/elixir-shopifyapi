defmodule ShopifyAPI.Shop do
  @derive {Jason.Encoder, only: [:myshopify_domain]}
  defstruct myshopify_domain: ""

  @typedoc """
  Type that represents a Shopify Shop with

    - myshopify_domain corresponding to the full myshopify hostname for the shop
  """
  ...
  def post_login(%ShopifyAPI.UserToken{} = token),
    do: :post_login |> shop_config() |> call_post_login(token)
  ...
  def slug_from_domain(myshopify_domain),
    do: String.replace(myshopify_domain, "." <> @shopify_domain, "")

  @spec to_uri(String.t()) :: URI.t()
  @spec to_uri(t()) :: URI.t()
  def to_uri(%__MODULE__{myshopify_domain: myshopify_domain}), do: to_uri(myshopify_domain)
